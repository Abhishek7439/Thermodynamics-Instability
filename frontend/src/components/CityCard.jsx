...
import { ComposedChart, Area, Bar, Tooltip, ResponsiveContainer } from 'recharts';
...
const CityCard = ({ data }) => {
  ...
  useEffect(() => {
    if (isHovered && !forecast) {
      fetchForecast(data.city).then(res => setForecast(res));
    }
  }, [isHovered, data.city, forecast]);
        ...
        <div className="relative z-10">
          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <div className="text-[10px] text-gray-400">Max Temp {data.temperature?.maxDeparture > 0 ? `(+${data.temperature.maxDeparture})` : `(${data.temperature?.maxDeparture ?? 0})`}</div>
            </div>
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            className="absolute z-50 w-72 glass-card p-4 shadow-2xl top-full mt-2 left-1/2 -translate-x-1/2 pointer-events-none"
            style={{ border: `1px solid ${sc.border}`, background: 'rgba(10, 22, 40, 0.95)', backdropFilter: 'blur(20px)' }}
          >
            <h4 className="text-white font-bold text-sm mb-3 flex items-center justify-between">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: sc.bg, color: sc.text }}>
                {data.analysis?.alertLevel ?? 'UNKNOWN'} ALERT
              </span>
            </h4>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              ...
              <div className="bg-blue-900/30 p-2 rounded-lg">
                ...
                <div className="text-white text-xs font-bold">{data.rainfall?.last24h ?? 0}mm</div>
              </div>
            </div>
              ...
              <div className="mb-2">
                ...
                <div className="h-16 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={forecast} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                      <defs>
                      ...
                      <Tooltip 
                        ...
                        formatter={(value, name) => [name === 'displayRainfall' ? `${value}mm` : `${value}°C`, name === 'displayRainfall' ? 'Rainfall' : 'Temp']}
                      />
                      <Bar dataKey="displayRainfall" fill="#3b82f6" opacity={0.6} radius={[2, 2, 0, 0]} />
            ...
            <div className="text-[9px] text-center text-gray-400 mt-2">
              Forecast Confidence: <span className="text-green-400">{data.analysis?.forecastConfidence ?? 'N/A'}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
